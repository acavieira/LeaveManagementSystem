...
public class LeaveTypeService(ApplicationDbContext _context, IMapper _mapper) : ILeaveTypeService
{
    ...
    public async Task<bool> CheckIfLeaveTypeExists(string name)
    {
        ...
        return await _context.LeaveTypes.AnyAsync(x => x.Name.Equals(lowercaseName));
    }

    public async Task<bool> CheckIfLeaveTypeExistsForEdit(LeaveTypeEditVM leaveTypeEdit)
    {
        var lowercaseName = leaveTypeEdit.Name.ToLower();
        return await _context.LeaveTypes.AnyAsync(x => x.Name.Equals(lowercaseName) && x.Id != leaveTypeEdit.Id);
    }
